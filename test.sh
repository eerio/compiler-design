...
for filename in $(find  "$RT2"/good/  "$RT"/good/basic ! -path "$RT/good/virtual/*" ! -path "$RT/good/arrays/*" ! -path "$RT2"/good/daria-error.lat ! -path lattests/extensions -name "*.lat"); do
  [ -e "$filename" ] || continue
  echo -n "$filename"
