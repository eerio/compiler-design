...
for filename in $(find  varqox/mrjp-tests/good mrjp-tests/good/ mrjp-tests/gr5/ lattests/extensions lattests/good/  ! -path "$RT2"/good/daria-error.lat ! -path mrjp-tests/good/hardcore/tail_call_optymization.lat -name "*.lat"); do
  [ -e "$filename" ] || continue
  echo -n "$filename"
