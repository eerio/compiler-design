...
for filename in $(find lattests/extensions/objects1 "$RT2"/good/ "$RT"/good/basic ! -path "$RT/good/virtual/*" ! -path "$RT/good/arrays/*" ! -path "$RT2"/good/daria-error.lat -name "*.lat"); do
  [ -e "$filename" ] || continue
  echo -n "$filename"
