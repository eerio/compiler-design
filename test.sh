...
for filename in $(find "$RT2"/opt "$RT2"/extensions/objects1 "$RT2"/extensions/objects2 "$RT2"/extensions/struct "$RT2"/extensions/shapes.lat -name "*.lat"); do
  [ -e "$filename" ] || continue
  ...
  # check if first line of stderr is "OK":
  if [ "`head -n 1 <&${fd_errr}`" != "OK" ]; then
